newFilter <- function(mapFilter,hideCountry,countryFilter,crossFilter,citation,year,gsRank,authors,university,publisher,keywordList){
  ...
  for (i in 1:nrow(midparticipatory)){
    countryISO2name <- midparticipatory[i,"ISO2.x"] %>% as.character()
    midparticipatory$Count[i] <- midparticipatory$Count[i] + str_count(crossarray,pattern=countryISO2name)
  }
  #print(midparticipatory$Count)
